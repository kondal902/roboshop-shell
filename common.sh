...
schema_setup() {
  if [ "${schema_type}" == "mongo" ]; then
    ...
    mongo --host mongodb-dev.devopsb71.xyz </app/schema/${component}.js &>>${log_file}
    status_check $?
    ...
    print_head "Load Schema"
    mysql -h mysql-dev.devopsb71.xyz -uroot -p${mysql_root_password} < /app/schema/shipping.sql &>>${log_file}
    status_check $?
  fi
}
